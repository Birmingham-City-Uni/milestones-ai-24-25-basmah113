...
public class EnemyController : MonoBehaviour
{
	...
	public Transform player, _camCallingBats;
	public NavMeshAgent enemy;
	public AISTATE enemyState = AISTATE.PATROL;
	public float distanceOffset = 2f;
	public float detectionRange, AttackRange,distanceToMove;
	public Animator _anim;
	public List<Transform> waypoints = new List<Transform>();
	private Transform currentWaypoint;
	public bool isAttack, isDie,_isHeathReduce;
	bool _iscalling;
	...
	void Update()
	{
		if (!isDie && !_isHeathReduce && !_iscalling)
		{
			CheckPlayerInRange();
		...
		if(_isHeathReduce && !isDie)
        {
			...
			targetPosition = transform.position + directionAwayFromPlayer * distanceToMove;

			transform.position = Vector3.MoveTowards(transform.position, targetPosition, 30 * Time.deltaTime);

			float _dis = Vector3.Distance(transform.position, player.position);
            if (_dis > detectionRange)
            {
				isOnce = false;
				_anim.SetBool("OffCall", false);
				enemyState = AISTATE.PATROL;
				ChangeState(enemyState);
				enemy.isStopped = true;
                _isHeathReduce = false;
				StartCoroutine(Ie_CamCallingBats());
            }
        }
	}
	...
	bool isOnce;
	 IEnumerator Ie_CamCallingBats()
	{
		if (isOnce == false)
		{
			isOnce = true;

			_camCallingBats.gameObject.SetActive(true);
			yield return new WaitForSecondsRealtime(0.2f);
			_anim.SetTrigger("CallingBats");
			yield return new WaitForSecondsRealtime(2f);
			GetComponent<BatSpawner>().FncSpawn();
			yield return new WaitForSecondsRealtime(3f);
			_iscalling = false;
			_camCallingBats.gameObject.SetActive(false);
			_anim.SetBool("OffCall", true);
		
			;
			GetComponent<HealthManager>().IncreaseHealth(50);
			enemy.isStopped = false;
			if (waypoints.Count > 0)
			{
				currentWaypoint = waypoints[Random.Range(0, waypoints.Count)];
